...
class ASPPPooling(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = ASPPpart(in_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1)
...
class ASPP(torch.nn.Module):
    def __init__(self, in_channels, out_channels, rates=(3, 6, 9)):
        super().__init__()
        branches = [ASPPpart(in_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1)]
        for rate in rates:
            branches.append(ASPPpart(in_channels, out_channels, kernel_size=3, stride=1, padding=rate, dilation=rate))
        branches.append(ASPPPooling(in_channels, out_channels))
        self.branches = torch.nn.ModuleList(branches)
        self.conv_out = ASPPpart(
            len(branches) * out_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1
        )
